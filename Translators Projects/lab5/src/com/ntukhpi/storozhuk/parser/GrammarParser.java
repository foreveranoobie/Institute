...
public class GrammarParser {
    ...
    public void parseRule(String line, Set<String> terminals, Set<String> nonTerminals, List<GrammarRule> rules) {
        String left = line.split("->")[0];
        String[] right = line.split("->")[1].split("\\|");
        if (right.length > 1) {
            for (String subRule : right) {
                parseRule(left.concat("->").concat(subRule), terminals, nonTerminals, rules);
            }
        } else {
