...
class Calculator{
    public:
        void calculateExpression(string* expression){
            ...
            for(int index = 0; index < expression->length() && expression->size() != 1 && stringContainsOperation(*expression); index++){
                while(!isOperation(expression, index)){
                    index++;
                }
                ...
                beginIndex = index;
                int erasureLimit = isUnaryOperation(operation) ? 2 : 3;
                while(beginIndex > -1 && count < erasureLimit){
    ...
    private:
        ...
        bool stringContainsOperation(string exp){
            return exp.find(" +") != -1 || exp.find(" -") != -1 || exp.find(" *") != -1 || exp.find(" /") != -1 || exp.find(" s") != -1;
        }
    
        float* getNumbers(string* expression, int index){
            string* number = new string;
            float* returnNumber = new float[2];
            while(index > -1 && (isNumber(expression->at(index)) || isNegativeNumber(expression, index))){
        ...
        bool isOperation(string* expression, int index){
            char symbol = expression->at(index);
            return !isNegativeNumber(expression, index) && (symbol == '*' || symbol == '/' || symbol == '+' || symbol == '-' || symbol == 's');
        }
